# 共享记忆安全审计 Agent 平台 · 知识手册

> **一份"由读到懂、由懂到会"的完整导引。**
> 把整套安全运营平台摊开来讲——从一行日志落到 Kafka 开始，到 iptables 把 C2 IP 拒之门外为止。
> 配合 [`00-overview.md`](./00-overview.md)–[`10-faq.md`](./10-faq.md) 的分章细读，本手册给出连贯的脉络。

| 项目 | 信息 |
| :--- | :--- |
| 受众 | 想从 0 到 1 掌握本平台架构、模块、代码与部署的工程师 / 学生 / 评委 |
| 时长 | 通读 1–2 小时；精读 + 动手 1–2 周 |
| 配套 | `docs/study-guide/` 12 篇分章节文档；`docs/deployment.md` 部署手册；`docs/advanced_capabilities.md` 高级能力 |
| 源码版本 | 与 `docker-compose.yml` 镜像版本一致；最近重审 2026-08-25 |
...
| 卷 | 主题 | 你将学会 |
| :--- | :--- | :--- |
| **上卷** | 缘起与架构 | 项目定位、5 层职责、Kafka 7 个 Topic、一条事件的完整旅程 |
| **中卷** | 流水线与模块 | Audit-LLM 4 层、CAD 监督、Sigma/RAG、响应引擎、5 道自审计闸 |
| **下卷** | 实战与附录 | 跑通链路、追一条事件、改一条规则、查一份排错、读懂新增模块 |

> **阅读方式**：第一次接触，请按目录顺序读；想速通，先看 [第一章](#第一章-它是什么) + [第三章](#第三章-一条事件的完整旅程)；想直接改代码，跳到 [中卷第四章 Audit-LLM](#第四章-audit-llm-四层流水线) 和 [下卷动手点](#动手点汇总)。
...
一句话讲清：**一套端到端的安全运营平台**——把"日志接入 → 校验认证 → 异常评分 → 攻击链 CEP → LLM 深度审计 → 独立监督 → 响应执行 → 复盘反馈"这 8 件事串成一条流水线，全部跑在 Docker 化的 Kafka + Flink + FastAPI + React 集群上，`docker compose up` 一键启动。

它不是 demo，不是论文截图，更不是把 LLM 摆在中间让人写规则的"AI 玩具"。它是一台"安全运营机器"——SOC 工程师每天面对的告警、审计、响应、复盘工作，都被压成可一键启动的工程产物。

**为什么不用"单 LLM Agent"？**

行业里常见两种思路：传统 SIEM（强在检索与合规，弱在 AI 原生）和单 LLM Agent（强在灵活，弱在实时、弱在可控）。本平台选择第三条路——**把 LLM 嵌入到流处理流水线里，且让它全程被监督**。这有三点根本不同：

1. **实时性**：Kafka + Flink 做秒级流式处理，不是"事件来了再调 LLM"
2. **可审计性**：4 层流水线 + CAD 独立监督，每层产物都可回查
3. **平台自安全**：5 道闸挡 AI 幻觉、越权、危险操作，平台自身不是裸的

**和"v1.3.0"的关系**

v1.3.0 是这套平台的前身——一个 MCP 控制网关（4 层 Guard）。本平台**集成**了 v1.3.0 的 Guard 作为安全中间件，并扩展出完整的 SOC 流水线。详细对比见 `D:\揭榜挂帅\v1.3.0_vs_platform_对比分析.md`。

---

## 第二章 · 5 层职责与全貌

平台不是"一个服务"，而是一组**有清晰边界的进程**，通过 Kafka 与 REST/SSE 通信。把它拆成 5 层，每层职责分明：
...
# 观测
prometheus / grafana / tempo / otel-collector / jaeger
```
...
> 这一章追一条 **"192.168.1.100 触发 50 次 SSH 登录失败 → 暴力破解告警"** 的全过程。
...
> **重要修复**（2026-08-23 审计发现）：早期版本下，高严重事件只走 `alerts` 不走 `audit-queue`，导致日志中心 80% 高危事件卡在"待审计"。修复见 [第十三章 · 历史教训与重要修复](#重要修复日志中心待审计问题闭环) 与 `audit/02-日志中心根因排查报告.md`。
...
② ToolBuilder  （agent_tool_builder.py）
   ...
   · 输出：工具调用计划
   ↓
③ Executor  （agent_executor.py，最大模块）
...
> 这是后端最大、最复杂的一块——把"让 LLM 审安全事件"这件难事拆成 4 层、让每层都有产物可校验、被 CAD 监督、被 Grounding 验证。
...
```
backend/prompts/
├── decomposer_initial_analysis.j2       + system
├── executor_deep.j2                      + system
├── executor_recheck.j2                   + system
├── executor_synthesize.j2                + system
├── reviewer_review.j2                    + system
├── sub_auditor_prompt.j2                 + system
├── evidence_verify.j2                    + system
├── rerank.j2                             + system
└── ... 共 46 个 .j2 文件
```

`tests/test_prompts_integrity.py` 保证 0 硬编码——所有 prompt 走 `loader.py` 加载，避免"代码里塞 LLM prompt"导致审计盲区。

### 旁路：Sub-Auditor

除了主流水线，还有一个**平行子审计** (`sub_auditor.py`)——从另一个视角独立审计，与主流水线结果交叉验证。当两者结论一致时，置信度上升；不一致时触发人工复核。

---

## 第五章 · CAD 独立监督与 Grounding
...
完整 8 条见 `backend/response_engine/response_policies.py`。
...
> 平台用 Apache Flink 1.19.3（Java 11）做实时流处理（实测 `flink-jobs/pom.xml` `<flink.version>1.19.3</flink.version>`，Kafka 连接器 `3.2.0-1.19`）。本章讲清两个核心作业、三个辅助作业、CEP 模式、数据契约。
...
辅助作业：
- `FlowAggregationJob`：把 NDR raw flow 聚合成 5min 滑窗（需 `SUBMIT_NDR_JOBS=1`）
- `SigmaThresholdAggregationJob`：Sigma 命中做阈值聚合去抖（默认启动，第三个作业）
...
CEP 模式可热更新——通过 `CepPatternConfig` + Kafka Broadcast State 推送到 Flink，**不需要重启 Flink** 就能改模式。
...
# 1. 复制环境变量模板
cp .env.example .env
# 填入：SHARED_MEMORY_LLM_API_KEY / SHARED_MEMORY_JWT_SECRET / POSTGRES_PASSWORD 等

# 2. 生成 Flink Dashboard 的 Basic Auth 凭证
...
- Flink Dashboard <http://localhost:3002>
...
# 注入 3 条故意有问题的 LLM 审计
# 观察 CAD 报告与熔断器状态变化
# 查状态：curl http://localhost:8001/api/cad/circuit-breaker
```
...
> 在深度审核中发现的、值得特别说明的模块，按职责域分组。

### 10.1 LLM 上下文工程

| 模块 | 职责 | 关键类 |
| :-- | :-- | :-- |
| `sliding_window.py` | 滑动窗口管理对话上下文 | `SlidingWindow` |
| `summary_compression.py` | 历史摘要压缩（防 token 爆） | `summary`, `embedder`, `cost_tracker` |
| `hierarchical_attention.py` | 层次化注意力（核心事件优先） | `HierarchicalAttention` |
| `context_stream.py` | 实时上下文流（Redis 支撑） | `ContextStream` |
| `memory_tree.py` | 层次化记忆树 | `MemoryTree` |

成本控制：每日 token 用量从 `llm_traces` 表**冷启动重建**，重启 backend 不丢预算门禁。

### 10.2 流量与协议解析

| 模块 | 职责 |
| :-- | :-- |
| `traffic_capture/` | 抓包引擎 + PCAP 存储 + Flow 聚合 |
| `protocol_parser/` | 协议解析器：DNS / HTTP / SMB / TLS |
| `traffic_baseline/` | 流量基线（季节性 / 同组对标 / LLM 异常识别） |
| `encrypted_traffic/` | TLS 指纹（JA3/JA4）+ 证书分析 + MITM 代理 |
| `session_reconstruct/` | TCP 重组 / HTTP 关联 / 时间线重建 |

### 10.3 高级安全能力

| 模块 | 能力 | 默认开关 |
| :-- | :-- | :-- |
| `edr_fusion/` | Sysmon / WinEvent 解析 + 跨源关联 | `edr_enabled` |
| `threat_intel/` | IOC 匹配 + MISP/TAXII 拉取 + 信誉评分 | `intel_enabled` |
| `zeroday_detect/` | CAPE 沙箱连接 + 行为分析 + 变体聚类 | `sandbox_enabled` |
| `phishing_guard/` | 7 类检测器（邮件 / 链接 / 附件 / 二维码 / BEC / 域名 / 短信） | `llm_phishing_enabled` |
| `ids_connector/` | Suricata IDS 对接 | 随 EDR |
| `data_security/` | LLM 驱动的敏感数据分类（识别 PII） | 默认开 |
| `ops_metrics/` | KPI 计算 + SLA 跟踪 | 默认开 |

### 10.4 评估与测试支撑

| 模块 | 用途 |
| :-- | :-- |
| `eval_service.py` + `eval_repository.py` | 审计准确率评估（offline） |
| `eval_metrics.py` | 评估指标计算（precision/recall/F1） |
| `audit_schemas.py` | 审计数据结构定义 |
| `audit_types.py` | 审计类型枚举 |
| `audit_trail.py` | 审计轨迹（业务事件） |
| `trace_context.py` + `trace_hook.py` | OTel trace 注入 |

### 10.5 工单与运营

| 模块 | 职责 |
| :-- | :-- |
| `work_order_service.py` | 工单流转、优先级、SLA |
| `case_manager.py` | 案例管理、经验复用、关联关系 |
| `post_mortem_service.py` | Post-Mortem 时间线 + 根因分析 |
| `feedback_loop.py` | 反馈闭环（误报抑制 / 漏报补偿） |
| `field_cipher.py` | 敏感字段落库脱敏 |
| `scheduler.py` | 定时任务调度 |
...
- [ ] `.env` 已设置全部 `:?` 必填项
- [ ] `bash tools/gen-htpasswd.sh admin your-password` 已生成凭证
- [ ] `bash tools/gen-kafka-certs.sh` 已生成 TLS 证书
- [ ] `bash tools/create-kafka-users.sh soc-log-source <password>` 已创建外部账号
- [ ] `docker compose up -d --build` 全部 healthy
- [ ] Flink 作业已提交并 RUNNING
- [ ] 外部验证：Redis 6380 不可达、Flink 8081 不可达、3001 无 Nginx 版本号、3002 未登录 401
- [ ] 注入测试数据看前后端正常
...
| 日志中心 80% 高危事件卡在"待审计" | Flink 路由 bug（已修复） | 详见 [第六章](#重要修复日志中心待审计问题闭环) |
...
> 这一章记录在审计中发现的"差点翻车"的修复点。读完后你会理解**为什么有些设计要这么写**。

### 重要修复：日志中心"待审计"问题闭环

**问题**（2026-08-23 发现）：

平台统计 `security_events=309, pending=249, completed=60`——**约 80% 事件停留在待审计**。

**根因**（三处叠加）：

1. **Flink 路由把高严重/高异常事件分流到"响应路径"而非"审计路径"**
   `AnomalyDetectionJob.java` 中：
   ```java
   boolean shouldAlert = (anomalyScore >= 0.6 || isHighSeverity) && !inColdStart;
   if (shouldAlert)            { ctx.output(ALERT_TAG, ...);      }   // 响应路径
   else if (score>=0.3 && !cs) { ctx.output(AUDIT_TAG, ...);      }   // 审计路径
   ```
   由于 `shouldAlert` 优先且为 `if/else if`，**高异常或 critical/high 的事件只会进 `security-alerts`，绝不会进 `security-audit-queue`**。

2. **backend 的 alert 消费处理不做审计**
   `kafka_consumer._handle_alert` 只做了两件事：发布 `alert` 到 event_bus、触发 `response_orchestrator.on_threat_detected`。**没有调用 Audit-LLM 流水线，也不把对应 `SecurityEvent.analyzed` 置为 true。**

3. **前端日志中心的审计状态只看 `analyzed` 字段**
   `Logs.tsx` 用 `log.analyzed` 渲染"✓ 已审计 / ● 审核中"。因此凡走 Kafka 高严重路径的事件永远是"审核中/待审计"。

**修复**（已在主分支）：

`backend/kafka_consumer.py` 的 `_handle_enriched`：
- 新增 `_queue_audit(...)` 辅助方法，复刻 `_handle_audit` 的 Audit-LLM 排队逻辑
- 在 `_handle_enriched` 存储并发布 `security_event` 后，对命中告警路径的事件（`anomaly_score >= 0.6` 或 `severity in (critical, high)`）同样排队审计

附带：修复 backend 启动中 `.pop("server", None)` 在旧版 starlette `MutableHeaders` 上抛 500 的问题。

**验证**（真实数据注入前后对照）：

| 事件 | 路径 | analyzed | llm |
| :-- | :-- | :-- | :-- |
| #330–#332（BRUTE_FORCE high, score=1.0） | 修复前 | ❌ False | ❌ |
| #334–#337（BRUTE_FORCE high, score=1.0） | 修复后 | ✅ True | ✅ |

日志证据：`[Kafka-Enriched-Audit] Queued event #334..337 for Audit-LLM (alert-path closure): BRUTE_FORCE score=1.00`。

**说明**：
- 该修复对**新接入**的高优先级事件生效
- 修复前已卡在"待审计"的历史事件不会自动补审（若需批量补审可作为后续任务）
- 详细报告：`audit/02-日志中心根因排查报告.md`

### 其他已修复问题

1. **git-bash curl 502 假象**：git-bash/MSYS curl 对容器映射端口返回 502，但浏览器/PowerShell/容器内均正常。排查环境问题时勿据 git-bash curl 误判为服务故障。
2. **审计状态口径**：把"响应已执行"也计入日志中心状态展示，区分 `analyzed`(LLM) 与 `responded`(响应)。
3. **鉴权补齐**：为 logs/SSE 端点补 `get_current_user`；SSE 改为从 `Authorization` 头解析 token。

### 当前 P0/P1 风险（开放中）

> 这些是 2026-08 桌面级审查时点出的开放项，详见 `audit/00-项目状态桌面审查.md`。

- **P0-1 鉴权覆盖率有缝**：`routers/audit.py` / `routers/ops.py` / `routers/kafka.py` 部分端点未显式声明 `Depends(get_current_user)`
- **P0-2 暴力破解防护缺失**：`/api/auth/login` 没有失败次数锁定、IP 级失败计数、验证码
- **P0-3 API 文档外暴露**：`/docs` `/redoc` `/openapi.json` 生产环境应默认关闭
- **P0-4 存储型 XSS**：日志/对话渲染需前后端联合修
- **P1-1 CI/CD 缺位**：测试目录与 GitHub Action 串联未确认
- **P1-2 SSE 长连接无重试/限流**：多端订阅会拉爆后端 worker
...
| 概念 | 一句话 | 出现位置 |
| :-- | :-- | :-- |
| **Kafka Topic** | 事件流转的"管道"，7 个分级 topic | 第二章 / 第三章 |
| **Flink 作业** | 跑在集群上的 Java 流处理 | 第八章 |
| **CEP 攻击链** | 模式匹配：扫描→爆破→C2 这种顺序事件 | 第八章 |
| **Audit-LLM** | 4 层 LLM 流水线：拆解→拼工具→执行→复核 | 第四章 |
| **CAD** | 独立监督 Agent，验证 LLM 是不是在"胡扯" | 第五章 |
| **Sigma 规则** | YAML 写的"如果…就…"检测规则 | 第七章 |
| **RAG** | 用 MITRE / CAPEC 知识库给 LLM 补背景 | 第七章 |
| **响应引擎** | 真正去开防火墙、封 IP 的执行层 | 第六章 |
| **MCP Guard** | 4 层检查：白名单 → 权限 → 参数 → 策略 | 第五章 |
| **Stabilizer** | 把 LLM 的"脏"输出修成干净的工具调用 | 第五章 |
| **Grounding** | 反幻觉：3 层证据校验 | 第五章 |
| **trace_id** | 贯穿全链路的"案卷号"，每条事件都有 | 第三章 |
| **NDR** | Network Detection & Response（流量抓包） | 第十章 |
| **EDR** | Endpoint Detection & Response（终端检测） | 第十章 |
| **TTL Manager** | 临时封禁的过期回收 | 第六章 |
| **rollback_token** | 每个响应动作的可撤销凭证 | 第六章 |
...
├── grounding_verifier.py           ← 反幻觉（3 层，42 KB）
│
...
> 完整列表：`curl http://localhost:8001/openapi.json`（约 179 个端点）
...
> 每一章都给了具体的"动手实验"。这里汇总成一张表，按"上手难度 × 收益"排序。

| # | 动手点 | 难度 | 章节 | 收益 |
| :--: | :-- | :--: | :-- | :-- |
| 1 | 跑一遍 `docker compose up -d --build` | ⭐ | 第九章 | 看到 7 个容器 healthy |
| 2 | 注入一次攻击链 | ⭐ | 第三、九章 | 看到一条事件走完 7 个 topic |
| 3 | 在 Jaeger 用 trace_id 追一条 | ⭐ | 第三章 | 理解 trace_id 贯穿 |
| 4 | 改一条 Sigma 规则 + reload | ⭐⭐ | 第七、九章 | 看 Sigma 热更新 |
| 5 | 加一条 CEP 模式（不重启 Flink） | ⭐⭐ | 第八章 | 看 Broadcast State 推送 |
| 6 | 触发熔断器 OPEN | ⭐⭐ | 第五、九章 | 理解 CAD 熔断状态机 |
| 7 | 让 LLM 试着注入（chat 关防火墙） | ⭐⭐ | 第五章 | 看到 5 道闸联动 |
| 8 | 找一次响应执行 + rollback | ⭐⭐ | 第六章 | 看 TTL 自动回收 |
| 9 | 加一个端到端测试 | ⭐⭐⭐ | 第十四章 | 路径 B 实战 |
| 10 | 加一个反钓鱼检测器 | ⭐⭐⭐ | 第十章 | 路径 B 实战 |
| 11 | 拆 Backend 为微服务 | ⭐⭐⭐⭐ | 第十四章 | 路径 D 改造 |
| 12 | 修一个 P0 风险（鉴权/暴力破解） | ⭐⭐⭐ | 第十三章 | 平台安全加固 |
...
- ✓ 知道平台做什么（上卷第一章）
- ✓ 知道整体怎么搭起来（上卷第二章 + 下卷第九章）
- ✓ 知道一条事件怎么走完一生（上卷第三章 + 中卷第四章）
- ✓ 知道后端每个模块干什么（中卷第四到七章 + 第十章）
- ✓ 知道 Flink 怎么写（中卷第八章）
- ✓ 知道 5 道自审计闸（中卷第五章）
- ✓ 知道前端怎么展示（第十六章）
- ✓ 知道数据怎么存（下卷第十七章）
...
- 选 [第九章](#第九章-跑起来追一条改一处) 的 1 个动手点
- 选 [第十四章 路径 B 实战](#路径-b--改它3-7-天) 的 1 个实战项目
- 选 [第十三章 开放风险](#当前-p0p1-风险开放中) 的 1 个 P0 项修一下

每天 1 个动手点，每周 1 个实战，每月 1 次全链路 demo 演练，季度 1 次升级演练。

> **最后的话**：这套平台的真正价值不是"用了多少技术"，而是**把 LLM 真正用到了生产场景且不让它失控**。
> 5 道闸不是噱头，CAD 不是装饰，trace_id 不是花架子——它们是平台**对自身安全性的承诺**。
> 读懂这些，你才算真正读懂了这个项目。
...
| 文档 | 用途 |
| :-- | :-- |
| [`README.md`](./README.md) | 知识手册入口（章节总览） |
| [`速查.md`](./速查.md) | 5 分钟速通（评委视角） |
| [`00-overview.md`](./00-overview.md) | 项目地图 |
| [`01-architecture.md`](./01-architecture.md) | 架构总览 |
| [`02-data-flow.md`](./02-data-flow.md) | 数据流与事件生命周期 |
| [`03-backend-modules.md`](./03-backend-modules.md) | 后端核心模块 |
| [`04-flink-jobs.md`](./04-flink-jobs.md) | Flink 流处理作业 |
| [`05-self-audit.md`](./05-self-audit.md) | 平台自审计体系 |
| [`06-frontend.md`](./06-frontend.md) | 前端架构与页面 |
| [`07-data-model.md`](./07-data-model.md) | 数据模型与消息契约 |
| [`08-deployment.md`](./08-deployment.md) | 部署、运维与调优 |
| [`09-learning-path.md`](./09-learning-path.md) | 推荐学习路径 |
| [`10-faq.md`](./10-faq.md) | 常见问题与陷阱 |
| [`../deployment.md`](../deployment.md) | 部署手册（端口矩阵、TLS、CI/CD） |
| [`../advanced_capabilities.md`](../advanced_capabilities.md) | NDR/EDR/威胁情报/0day/反钓鱼 |
| [`../upgrade-proposals/2026-q3-tech-stack-upgrade.md`](../upgrade-proposals/2026-q3-tech-stack-upgrade.md) | Q3 技术栈升级提案 |
| [`D:\揭榜挂帅\v1.3.0_vs_platform_对比分析.md`](D:\揭榜挂帅\v1.3.0_vs_platform_对比分析.md) | 与前身 v1.3.0 对比 |
| [`D:\揭榜挂帅\audit\00-项目状态桌面审查.md`](D:\揭榜挂帅\audit\00-项目状态桌面审查.md) | 桌面级现状审查 |
| [`D:\揭榜挂帅\audit\02-日志中心根因排查报告.md`](D:\揭榜挂帅\audit\02-日志中心根因排查报告.md) | 日志中心根因排查 |
...
> *本手册最后更新于 2026-08-25，与 `docker-compose.yml` 镜像版本同步。*
> *如果读完所有章节仍觉得"差点什么"，请翻 [第十三章 历史教训](#第十三章-历史教训与重要修复) —— 那里记录了本项目最真实的成长。*
